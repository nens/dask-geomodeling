...
Dask-geomodeling abstracts data layers ("blocks") that are to be subsetted later. Tiling can
be done explicitly by adding a `Tiler` block at the end. For instance:
...
graph is equivalent to the graph of a dask collection, but without the built-in chunking.
...
# Steps to take to use dask more directly

## Summary of intended usage

First we construct a dask collection without any chunks (1 chunk is also ok):
