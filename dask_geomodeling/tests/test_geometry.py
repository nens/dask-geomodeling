...
def create_geojson(
    abspath, polygons=10, bbox=None, ndim=2, projection='EPSG:4326'
):
...
class TestGeometryFileSource(unittest.TestCase):
    ...
    def test_get_data_centroid_mode(self):
        ...
        self.assertTrue(
            Polygon(*self.polygons).intersection(box(*self.bbox)))
        self.assertFalse(
            Polygon(*self.polygons).centroid.within(box(*self.bbox)))
        self.assertEqual(self.projection, result['projection'])
    ...
    def test_bbox(self):
        ...
        corner = np.array(
            [(0., 2.), (2., 2.), (2., 0.), (1.1, 0.), (1.01, 1.1), (0., 1.1)]
        )
        ...
        # square and edge
        result = self.source.get_data(
            geometry=box(0., 0., 1., 1.), projection='EPSG:4326'
        )
        self.assertEqual(2, len(result['features']))

        # only square
        result = self.source.get_data(
            geometry=box(0., 0., 0.9, 1.), projection='EPSG:4326'
        )
    ...
    def test_size_filter(self):
        full = (0., 0.), (0., 1.), (1., 1.), (1., 0.)
        small = (0., 0.), (0., 0.1), (0., 0.1), (0.1, 0.)
    ...
    def test_extent_mode(self):
        result = self.source.get_data(
            geometry=box(*self.bbox), projection='EPSG:4326'
        )
...
class TestSetOperations(unittest.TestCase):
    ...
    def test_intersect_with_request(self):
        ...
        self.assertAlmostEqual(1., result['features'].iloc[0]['geometry'].area)

        # return the intersected extent
        self.request['mode'] = 'extent'
        result = view.get_data(**self.request)
        self.assertTupleEqual((0., 0., 1., 1.), result['extent'])
    ...
    def test_difference(self):
        ...
        self.assertAlmostEqual(4., other_req['geometry'].area)

        # the result should be the difference between the two polygons
        result = view.get_data(**self.request)
        self.assertEqual(1, len(result['features']))
        self.assertAlmostEqual(3., result['features'].iloc[0]['geometry'].area)
    ...
    def test_difference_with_empty_other(self):
        ...
        result = view.get_data(**self.request)
        self.assertEqual(1, len(result['features']))
        self.assertAlmostEqual(4., result['features'].iloc[0]['geometry'].area)
    ...
    def test_area(self):
        ...
        self.assertListEqual(
            result.tolist(),
            [Polygon(x).area for x in self.polygons]
        )
...
class TestConstructive(unittest.TestCase):
    ...
    def test_min_size_simplify(self):
        trapezoid1 = (0., 0.), (0.49, 1.), (0.51, 1.), (1., 0.)
        trapezoid2 = (0., 0.), (0.4, 1.), (0.6, 1.), (1., 0.)
...
class BufferTestCase(unittest.TestCase):
    ...
    def test_buffer_transform(self):
        ...
        request = dict(
            mode='intersects',
            projection='EPSG:4326',
            geometry=box(4, 51, 7, 53),
        )
        data = source.get_data(**request)
        # Transform back to RD New for square meters.
        actual = data['features'].geometry.apply(
            shapely_transform, args=('EPSG:4326', 'EPSG:28992')
        ).area
...
class TestAggregateRaster(unittest.TestCase):
    ...
    def test_arg_types(self):
        self.assertRaises(
            TypeError, geometry.AggregateRaster, self.source, None
        )
        self.assertRaises(
            TypeError, geometry.AggregateRaster, None, self.raster
        )
        self.assertRaises(
            TypeError,
        ...
        self.assertEqual(1., view.pixel_size)
    ...
    def test_snap_bbox(self):
        for (x1, y1, x2, y2), exp_bbox, (exp_width, exp_height) in (
            ...
            _, (_, request), _ = \
                self.view.get_sources_and_requests(**self.request)
    ...
    def test_intensive_scaling(self):
        ...
        view1 = geometry.AggregateRaster(
            self.source,
            self.raster,
            statistic='mean'
        )
    ...
    def test_different_projection(self):
        ...
        self.assertEqual(result['features'].iloc[0]['agg'], 1.)
    ...
    def test_chained_aggregation(self):
        ...
        self.assertEqual(36., feature['agg'])
        self.assertEqual(7., feature['agg2'])
    ...
    def test_overlapping_geometries(self):
        ...
        result = view.get_data(**self.request)
        self.assertEqual(
            result['features']['agg'].values.tolist(),
            [36.0, 18.0],
        )
    ...
    def test_aggregate_above_threshold(self):
        ...
        source = MockGeometry(
            polygons=[
                ((2., 2.), (4., 2.), (4., 4.), (2., 4.)),  # contains 7, 8
                ((7., 7.), (9., 7.), (9., 9.), (7., 9.)),  # contains 2, 3
                ((6., 6.), (8., 6.), (8., 8.), (6., 8.)),  # contains 3, 4
            ],
            properties=[{'id': 1, 'threshold': 8.},  # threshold halfway
                        {'id': 2000000, 'threshold': 4.},  # threshold above
                        {'id': 9}],  # no threshold
        )
        self.request['start'] = Datetime(2018, 1, 1)
        self.request['stop'] = Datetime(2018, 1, 1, 3)
...
class TestSetGetSeries(unittest.TestCase):
    def setUp(self):
        ...
        self.source1 = MockGeometry(
            polygons=[((2., 2.), (8., 2.), (8., 8.), (2., 8.))] * self.N,
            properties=self.properties,
        )
        self.properties = [
            {'id': i, 'col_2': i * 3, 'col_3': i * 4}
            for i in range(self.N)
        ]
    ...
    def test_set_series(self):
        source = geometry.SetSeriesBlock(
            self.source1, 'added', self.source2['col_2']
        )
        ...
        self.assertSetEqual(
            {'geometry', 'col_1', 'added'}, source.columns
        )

    def test_set_series_overwrite(self):
        source = geometry.SetSeriesBlock(
            self.source1, 'col_1', self.source2['col_2']
        )
        data = source.get_data(**self.request)
        added_values = data['features']['col_1'].values
        assert_almost_equal(added_values, [i * 3 for i in range(self.N)])
        self.assertSetEqual(
            {'geometry', 'col_1'}, source.columns
        )
    ...
    def test_set_series_multiple(self):
        ...
        self.assertSetEqual(
            {'geometry', 'col_1', 'added', 'added2'}, source.columns
        )
...
class TestWhere(unittest.TestCase):
    def setUp(self):
        values = [-float('inf'), -2, 1.2, 5., float('inf'), float('nan')]
        self.properties = [
            {'id': i,
             'col_1': x,
        ...
        self.source = MockGeometry(
            polygons=[((2., 2.), (8., 2.), (8., 8.), (2., 8.))] * len(values),
            properties=self.properties,
        )
...
class TestMerge(unittest.TestCase):
    ...
    def setUp(self):
        polygon_1 = [((2., 2.), (3., 2.), (3., 3.), (2., 3.)), ]
        polygon_2 = [((3., 3.), (4., 3.), (4., 4.), (3., 4.)), ]
        polygon_4 = [((5., 5.), (6., 5.), (6., 6.), (5., 6.)), ]
    ...
    def test_merge_dask_geomodeling(self):
        view = merge.MergeGeometryBlocks(
            left=self.source_1, right=self.source_2,
    ...
    def test_merge_dask_geomodeling_empty_source(self):
        view = merge.MergeGeometryBlocks(
            left=self.source_1, right=self.source_3,
            how='inner', suffixes=('', '_right')
        )
        result = view.get_data(**self.request)
...
class TestFieldOperations(unittest.TestCase):
    ...
    def test_choose(self):
        series = field_operations.Choose(
            ...
            self.source['col_1'], self.source['col_2'], self.source['bool_1']
        )
    ...
    def test_choice_dtype_str(self):
        series = field_operations.Choose(
            self.source['id_value'],
            self.source['col_choice_1'], self.source['col_choice_2']
        )
        result = series.get_data(**self.request)
        values = result.values
    ...
    def test_choose_different_length(self):
        val = [-float('inf'), -2, 1.2, 5., float('inf'), float('nan'), 1]
        properties = [
            {'id_value': float(i)} for i, x in enumerate(val)
        ]
        source_2 = MockGeometry(
            polygons=[((2., 2.), (8., 2.), (8., 8.), (2., 8.))] * len(val),
    ...
    def test_classify_field(self):
        series = field_operations.Classify(
            self.source['col_1'], bins=[0, 1.2, 5.], labels=['A', 'B']
        )
        result = series.get_data(**self.request)
        values = result.values
    ...
    def test_classify_from_columns(self):
        source_with_bins = self.source.set(
            'bin_1', 0, 'bin_2', 1.2, 'bin_3', 5.
        )
        series = field_operations.ClassifyFromColumns(
            source_with_bins,
            'col_1',
    ...
    def test_mul_none_fields(self):
        ...
        result = view.get_data(**self.request)
        self.assertTrue(
            np.all(np.isnan(result['features']['result']))
        )
    ...
    def test_set_multiple_columns(self):
        ...
        result = view.get_data(**self.request)
        self.assertTrue(result['features']['sum'].equals(
            self.prop_df['col_1'] + self.prop_df['col_2']
        ))
        self.assertTrue(result['features']['product'].equals(
            self.prop_df['col_1'] * self.prop_df['col_2']
        ))
    ...
    def test_interp(self):
        ...
        view = field_operations.Interp(
            self.source['col_1'], [0., 5.], [0., 10.], left=-1., right=11.,
        )
        result = view.get_data(**self.request)
        values = result.values

        self.assertEqual(-1., values[0])  # -inf
        self.assertEqual(-1., values[1])  # -2
        self.assertEqual(2.4, values[2])  # 1.2
        self.assertEqual(10., values[3])  # 5.
        self.assertEqual(11., values[4])  # inf
        self.assertTrue(np.isnan(values[5]))  # nan
...
class TestParallelize(unittest.TestCase):
    ...
    def test_empty(self):
        view = parallelize.GeometryTiler(MockGeometry([]), 5., self.projection)
        data = view.get_data(**self.request)
        self.assertEqual(0, len(data['features']))
    ...
    def test_no_tiling(self):
        view = parallelize.GeometryTiler(self.source, 10, self.projection)
        requests = \
            [x[1] for x in view.get_sources_and_requests(**self.request)]
        self.assertEqual(1, len(requests))
        self.assertEqual(50., requests[0]['geometry'].area)
    ...
    def test_two_tiles(self):
        ...
        for request in requests:
            self.assertEqual(25., request['geometry'].area)
    ...
    def test_skip_empty_tiles(self):
        ...
        for request in requests:
            self.assertLess(request['geometry'].area, 25.)
    ...
    def test_tile_different_projection(self):
        ...
        for request in requests:
            ...
            self.assertAlmostEqual(25., request['geometry'].area)
...
class TestText(unittest.TestCase):
    def setUp(self):
        ...
        self.view = text.ParseTextColumn(
            self.source, 'description', self.key_mapping
        )
        self.request = dict(
            mode='intersects',
            projection='EPSG:3857',
    ...
    def test_parser_columns(self):
        ...
        self.assertTrue(
            set(self.key_mapping.values()).issubset(
    ...
    def test_parser_columns_empty_description(self):
        ...
        view = text.ParseTextColumn(
            source, 'description', self.key_mapping
        )
        data = view.get_data(**self.request)
        self.assertTrue(
            set(self.key_mapping.values()).issubset(
