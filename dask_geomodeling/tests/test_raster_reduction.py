...
def stack():
    ...
    return [
        ...
        {
            "values": np.array([[[3, 3, 3], [4, 42., 42.]]], dtype=np.float32),
            "no_data_value": 42.,
        },
    ]
...
def stack_nodata_only():
    return [
        {
            "values": np.zeros((1, 2, 3), dtype=np.uint8),
            "no_data_value": 0,
        },
        {
            "values": np.ones((1, 2, 3), dtype=np.uint64),
            "no_data_value": 1,
        },
        {
            "values": np.full((1, 2, 3), 42., dtype=np.float32),
            "no_data_value": 42.,
        },
    ]
...
def test_reduce(statistic, expected, dtype, stack):
    actual = reduce_rasters(
        stack, statistic, (1, 2, 3), fill_value=255, dtype=dtype
    )
    expected = np.array(expected, dtype=dtype)
    assert_array_equal(actual, expected)


@pytest.mark.parametrize("statistic,expected_value", [
    ("first", 255),
    ("last", 255),
    ("count", 0),
    ("nans", 0),
...
def test_reduce_no_input(statistic, expected_value):
    actual = reduce_rasters(
        [], statistic, (1, 2, 3), fill_value=255, dtype=np.uint8
    )
